...
public class Lz4Test extends TestParent {
	...
	public int testFastNativeDouble64K() throws IOException {
		...
		return baseBenchmark(new StreamFactory() {
			...
			public OutputStream getStream(OutputStream underlyingStream) throws IOException {
				return new LZ4BlockOutputStream(new LZ4BlockOutputStream(underlyingStream, BLOCK_64K, compressor),
						BLOCK_64K, compressor);
			}
		});
	}
	...
	public int testFastNativeDouble32M() throws IOException {
		...
		return baseBenchmark(new StreamFactory() {
			@Override
			public OutputStream getStream(OutputStream underlyingStream) throws IOException {
				return new LZ4BlockOutputStream(new LZ4BlockOutputStream(underlyingStream, MAX_BLOCK_SIZE, compressor),
		...
		return baseBenchmark(new StreamFactory() {
			...
			public OutputStream getStream(OutputStream underlyingStream) throws IOException {
				return new LZ4BlockOutputStream(
						new LZ4BlockOutputStream(new LZ4BlockOutputStream(underlyingStream, MAX_BLOCK_SIZE, compressor),
								MAX_BLOCK_SIZE, compressor),
						MAX_BLOCK_SIZE, compressor);
			}
		});
